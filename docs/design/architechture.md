...
- **TritonKernelCacheNodeStatus CRD (namespaced):**
  Reflects the observed state of kernel cache compatibility and readiness on a
  specific node, for a given TritonKernelCache. Created and updated by the TKM
  Agent. Tracks status per-GPU per-kernel image. Exists in the same namespace
  as the corresponding TritonKernelCache.
...
- **gpus:** A list of GPUs on the node, each with:
  - **id:** A unique identifier for the GPU (e.g., index).
  - **gpuType:** The type of GPU present.
  - **driverVersion:** Version of the GPU driver.
  - **caches**: A map of Triton kernel cache references to their per-GPU status,
    including:
    - **conditions**: Readiness and compatibility information.
    - **lastUpdated**: Timestamp of last update for this kernel on this GPU.
      ...
      caches:
        kernel-x:
          conditions:
            ...
            - type: Compatible
              status: "True"
          lastUpdated: "2025-05-29T18:00:00Z"
        kernel-y:
          conditions:
            - type: Ready
              status: "False"
              reason: Missing symbols
              message: Kernel not compatible with A100
            - type: Compatible
              status: "False"
          lastUpdated: "2025-05-29T18:01:00Z"
    - id: 1
      gpuType: nvidia-v100
      ...
      caches:
        kernel-x:
          conditions:
            ...
            - type: Compatible
              status: "False"
          lastUpdated: "2025-05-29T18:02:00Z"
```
...
```console
/etc/tkm/caches/<namespace>/<pod-name>/<cache-id>/
```
